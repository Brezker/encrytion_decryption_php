	...
	<body>
		...
		<main class="main">
			<div class="container">
				...
				$result = mysqli_query($conexion, "SELECT * FROM detallepedido WHERE cve_usuario =$id_cliente and pago='Pagado'");
				...
				<?php }else{ ?>
					<table class="table table-responsive text-nowrap">
						<thead>
							<tr>
								...
								<th>Revisar reportes</th>
							</tr>
						</thead>
						<?php 
						$sql_register = mysqli_query($conexion, "SELECT COUNT(*) as total_registros FROM detallepedido WHERE cve_usuario =$id_cliente and pago='Pagado'");
                    ...
                    $resultArchivos = mysqli_query($conexion, "SELECT * FROM archivo_informe WHERE nombre = '$Cotizacion_no' AND estatus = 'Aprobado'");
	                   ...
	                   echo 'Sin registros';
                     	...
                     	while ($datos = mysqli_fetch_array($resultArchivos)) {?>
                               ...
                               <form method="post"  action="ReportesCliente.php">
                               <input type="hidden" name="Cotizacion_no" value="<?php echo $data["Cotizacion_no"] ?>">
                               <button class="btn btn-lg" style="background-color:#1065c7; width:50px; height:38px" >
